...
#TODO: Make a new file for each location tested and each model used.
#TODO: Filter by synapse
#TODO: Make this project more generaliziable
...
for (model in models_vec) {
  ...
  for (species in species_vec) {
    ...
    pdf(file = paste0(output_species_dir, "AA_Fisher_Tests___Models-", paste0(models_vec, collapse="-"), 
                      "___Locations-", paste0(names(location_dict), "-")  ,".pdf"))
    ...
    for (location in names(location_dict)) {
      ...
      proteins_no_hmm <- annotate_cats(df = proteins_no_hmm, cats = location_cats)

      for (isoform_filter in c("on", "off")) {
        for (nuclear_filter in c("on", "off")) {
          for (neuron_filter in c("on", "off")) {
            ...
            for (i in 1:length(candidate_AA_vec)) {
              ...
              if (neuron_filter == "on") {
                proteins$nuclear <- unlist(
                  lapply(
                    lapply(as.character(proteins$go_ids_CC), function(x){unlist(strsplit(x, "; "))}),
                    function(x){any(x %in% neuron_cats)}
                  )
                )
                proteins = proteins %>% filter(proteins$nuclear == FALSE)
              }
              ...
              if (nrow(polyAA_df) == 0) {
                ...
                aa_fraction_df <- rbind(aa_fraction_df, data.frame(TractFraction = polyAA_df$MaxPolyAARegionAAFractions, AA = vec , loc_label = polyAA_df$label))
              }
            } # for (i in 1:length(candidate_AA_vec)) {
            ...
            p <- ggplot(p_value_hist_df, aes(x = names, y = -log(pvalues), fill = pvalues)) + 
              ...
              ggtitle(paste0("Enrichment of polyAA tracts in ", location, " Proteins by Fisher Test \n",
                             "Nuclear Filter: ", nuclear_filter, ", Isoform Filter: ", isoform_filter, " Neuron Filter: ", neuron_filter)) +
