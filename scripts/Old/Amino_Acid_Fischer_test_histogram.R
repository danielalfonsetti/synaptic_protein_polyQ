...
for (species in species_vec) {
  ...
  for (i in 1:length(candidate_AA_vec)) {
    ...
    if (sum(proteins$HMMhasPolyAA) > 0){
      p.val <- fisher.test(proteins$synaptic, proteins$HMMhasPolyAA)
    } else {
      p.val = 1
    }
  ...
  # png(paste0(output_base_dir, paste0("fisher_test_results_plots_", species, ".png")))
  ...
  # ggplot(df, aes(x = names, y = pvalues, fill = pvalues)) + 
  #   geom_col(alpha = .5, show.legend = FALSE) + 
  #   theme_light() +
  #   xlab("Amino Acid") +
  #   ylab("Fisher Test P-value") + 
  #   ylim(0, 1) +
  #   ggtitle("Enrichment of polyAA tracks in Synaptic Proteins by Fisher Test") +
  ...
  df = data.frame(names = factor(names(fisher_test_results), levels = names(fisher_test_results)),
                  pvalues = unlist(unname(fisher_test_results)))
  ggplot(df, aes(x = names, y = -log(pvalues), fill = pvalues)) + 
    geom_col(alpha = .5, show.legend = FALSE) + 
    theme_light() +
    xlab("Amino Acid") +
...
for (species in species_vec) {
  ...
  png(paste0(output_species_dir, paste0("fisher_test_results_plots_", species, "2.png")))
  ggplot(df2, aes(x = TractFraction)) +
    geom_histogram() +
    facet_wrap("AA") + 
