...
HiddenMarkov = function(proteinSet, candidate_AA){
  ...
  # Debugging
  # proteinSet = protein_df_w_seqs$peptide[1:1000]
  #
  # proteinSet <- protein_df_w_seqs[1:100,]
    # seqHMM library testing
    # proteinSet$peptide_seq
    ...
    list_seqs_split_padded <- lapply(list_seqs_split, function(lst) c(lst, rep(NA, maxlen - length(lst)))) # Pad
    
    split_seq_result <- do.call(rbind, list_seqs_split_padded)
    split_seq_result_df <- as.data.frame(split_seq_result)

    
    # split_seq_result_df
    
    # result <- cbind(proteinSet, split_seq_result_df)
    # View(result)
    
    # colnames(result) # Just want to get the first 18 columns. Columns 19:END contains sequence.

    # str(split_seq_result_df)
    ...
    hmm_fit <- fit_model(hmm, 
                         ...
                         threads = 10)
    print(paste0("Time taken: ", Sys.time() - start_time, " seconds."))
    ...
    hidden_paths_seqs <- hidden_paths(hmm_fit$model) # Computes Viterbi paths
  ...
  #################
    ...
    plot(hmm) # Intial hmm
    plot(hmm_fit$model) # Fit hmm
    ssplot(hmm_fit$model, plots = "both", border = NA)
    dev.off()
  ...
  # proteinSet = foreach(sequenceNum = 1:nrow(proteinSet),  .combine = 'rbind') %dopar% {
...
for (species in species_vec) {
  ...
  for (candidate_AA in candidate_AA_vec) {
    ...
    protein_df_w_hmm <- HiddenMarkov(protein_df_w_seqs, candidate_AA)
