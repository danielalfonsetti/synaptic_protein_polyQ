...
fisher_test_comparsion <- function(location_cats = NA, 
...
for (model in models_vec) {
  ...
  for (species in species_vec) {
    for (location in names(location_dict)) {
      ...
      for (isoform_filter in c("on", "off")) {
        for (nuclear_filter in c("on", "off")) {
          for (neuron_filter in c("on", "off")) {
            ...
            fisher_test_comparsion(location_cats, isoform_filter, nuclear_filter, neuron_filter, proteins_no_hmm, output_species_dir)
      ...
      dev.off()
    } #(location in names(location_dict)) {
  } #for (species in species_vec) {
} # for (model in models_vec) {
