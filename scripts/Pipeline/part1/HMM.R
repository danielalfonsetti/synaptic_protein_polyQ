...
for (model in kModels) {
  for (species in kSpecies) {
    ...
    for (candidateAA in kCandidateAAs) {
      print(paste0("CandidateAA: ", candidateAA))
      ...
      # Save
      write.csv(proteinDfHMMnucFilAltSpl, file = paste0(outputAADir, species, "_prots_w_HMM_", candidateAA, "_alt_spliced_nuclear_filt.csv"), row.names = FALSE)
