...
for (species in species_vec) {
  ...
  if (species == "fly") {
    ...
    ensembl_peptide_id <- names(as.list(org.Dm.egFLYBASEPROT2EG))
  } else if (species == "human")  {
    ensembl <- useMart("ensembl", dataset = "hsapiens_gene_ensembl")
    ensembl_peptide_id <- names(as.list(org.Hs.egENSEMBLPROT2EG))
  } else if (species == "mouse")
  {
    ensembl <- useMart("ensembl", dataset = "mmusculus_gene_ensembl")
    ensembl_peptide_id <- names(as.list(org.Mm.egENSEMBLPROT2EG))
  } else if (species == "worm")
  {
    ensembl <- useMart("ensembl", dataset = "celegans_gene_ensembl")
    ensembl_peptide_id <- names(as.list(org.Ce.egENSEMBLPROT2EG))
  }
  
  # Debugging
    ensembl_peptide_id <- ensembl_peptide_id[1:100]
