...
species = c("fly", "mouse", "human") # choose from 'fly', 'human', 'mouse' and 'worm'
...
for (species in species_vec) {
  ...
  if (species == "fly") {
    ...
    ensembl_peptide_id <- names(as.list(org.Dm.egFLYBASEPROT2EG))[1:200]
  } else if (species == "human")  {
    ensembl <- useMart("ensembl", dataset = "hsapiens_gene_ensembl")
    ensembl_peptide_id <- names(as.list(org.Hs.egENSEMBLPROT2EG))[1:200]
  } else if (species == "mouse")
  {
    ensembl <- useMart("ensembl", dataset = "mmusculus_gene_ensembl")
    ensembl_peptide_id <- names(as.list(org.Mm.egENSEMBLPROT2EG))[1:200]
  } else if (species == "worm")
  {
    ensembl <- useMart("ensembl", dataset = "celegans_gene_ensembl")
    ensembl_peptide_id <- names(as.list(org.Ce.egENSEMBLPROT2EG))[1:200]
  }
  
  
  # length(ensembl_peptide_id)
  # length(unique(ensembl_peptide_id))
  
  my_df <- getBM(attributes = c('ensembl_gene_id', 'ensembl_peptide_id', "external_gene_name", 'go_id', "name_1006", "namespace_1003"), 
                 filters = "ensembl_peptide_id", 
                 values = ensembl_peptide_id, 
                 mart = ensembl)
  my_df <- rename(my_df, "go_description" = name_1006 , "go_type" = namespace_1003 )
  ...
  write.csv(my_df, file = paste0(output_species_dir, "proteins", nrow(my_df),".csv"), row.names = FALSE)
  
  # Do the filtering for nuclear proteins here.
  # [FILTERING CODE]
  #
  
  seqs <- getSequence(id = ensembl_peptide_id, 
  ...
  my_df_2 <- merge(my_df, markov_result, by = "ensembl_peptide_id")
  my_df_2 <- my_df_2 %>% dplyr::select("ensembl_gene_id", everything())
  write.csv(my_df_2, file = paste0(output_AA_dir, "proteins", nrow(my_df_2), "_w_HMM_", candidate_AA, ".csv"), row.names = FALSE)
  ...
  # Load in protein set data from local storage if you didn't download the sets from above
    proteins <- read.csv(paste0(output_AA_dir, "proteins", nrow(my_df_2), "_w_HMM_", candidate_AA, ".csv"))
  #####
  ...
  helper_func <- function(fromType, OrgDb, proteins_unique, polyAA_proteins_unique) {
    ...
    sig_df <- bitr(polyAA_proteins_unique,
                   ...
                   toType = c("ENTREZID", "UNIPROT"),
                   OrgDb = OrgDb)
    return(list(universe_df, sig_df))
  }
  
  if (species == "fly") {
    # Contains all genes, not just significant prpoteins
    result <- helper_func("FLYBASEPROT", org.Dm.eg.db, proteins_unique, polyAA_proteins_unique)
    universe_df <- result[[1]]
    sig_df <- result[[2]]
  } else if (species == "human") {
    result <- helper_func("ENSEMBLPROT", org.Hs.eg.db, proteins_unique, polyAA_proteins_unique)
    universe_df <- result[[1]]
    sig_df <- result[[2]]
  } else if (species == "mouse") {
    result <- helper_func("ENSEMBLPROT", org.Mm.eg.db, proteins_unique, polyAA_proteins_unique)
    universe_df <- result[[1]]
    sig_df <- result[[2]]
  } else if (species == "worm") {
    result <- helper_func("ENSEMBLPROT", org.Ce.eg.db, proteins_unique, polyAA_proteins_unique)
    universe_df <- result[[1]]
    sig_df <- result[[2]]
  }
  ...
  for (go_ont in c("BP", "MF", "CC")) {
    ...
    if (nrow(go_enrich_output_control) != 0) {
      write.csv(go_enrich_output_control, file = paste0(output_AA_dir, "Control_Enriched_GO_", go_ont, ".csv"), row.names = FALSE)
    ...
    if (nrow(go_enrich_output) != 0) {
      write.csv(go_enrich_output, file = paste0(output_AA_dir, "Enriched_GO_", go_ont, ".csv"), row.names = FALSE)
      
      pdf(paste0("AA_", candidate_AA, "_Enriched_GO_", go_ont, "_plots.pdf"), width = 8.5, height = 11)
      par(mfrow = c(1, 1))
  ...
  ###########################################
  pdf(paste0(output_AA_dir, "HMM_", candidate_AA, "_result_plots_", nrow(proteins),"prots.pdf"))
  ...
  p <- ggplot(data = df, aes(x = df$AvgPolyAARegionAAFractions)) +
    geom_histogram(show.legend = FALSE, aes(fill=..count..)) +
