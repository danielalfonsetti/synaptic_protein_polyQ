...
FLYBASEPROT <- names(as.list(org.Dm.egFLYBASEPROT2EG))[1:1000]
length(FLYBASEPROT)
...
write.csv(my_df, file = paste0(output_species_dir, "proteins.csv"), row.names = FALSE)
...
my_df_2 <- my_df_2 %>% dplyr::select("flybase_gene_id", everything())




write.csv(my_df_2, file = paste0(output_AA_dir, "proteins_w_HMM_", candidate_AA, ".csv"), row.names = FALSE)
...
# Load in protein set data from local storage if you didn't download the sets from above
  proteins <- read.csv(paste0(output_AA_dir, "proteins_w_HMM_", candidate_AA, ".csv"))
#####
