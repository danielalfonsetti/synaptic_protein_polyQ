...
for (species in species_vec) {
  ...
  if (species == "fly") {
    file = getProteome(db = "ensemblgenomes", "Drosophila melanogaster", reference = TRUE,
                path = file.path("_ncbi_downloads", "proteomes"))
    ensembl <- useMart("ensembl", dataset = "dmelanogaster_gene_ensembl")
    
  } else if (species == "human")  {
    file = getProteome(db = "refseq", "Homo sapiens", reference = TRUE,
                       path = file.path("_ncbi_downloads", "proteomes"))
    ensembl <- useMart("ensembl", dataset = "hsapiens_gene_ensembl")
    
  } else if (species == "mouse") {
    file = getProteome(db = "refseq", "Mus musculus", reference = TRUE,
                       path = file.path("_ncbi_downloads", "proteomes"))
    ensembl <- useMart("ensembl", dataset = "mmusculus_gene_ensembl")
    
  } else if (species == "worm") {
    file = getProteome(db = "ensembl", "Caenorhabditis elegans", reference = TRUE,
                       path = file.path("_ncbi_downloads", "proteomes"))
    ensembl <- useMart("ensembl", dataset = "celegans_gene_ensembl")
  }
  AAStringSet <- read_proteome(paste0("C:/UROPs/polyQ_neuronal_proteins/", file), 
                               format = "fasta",
                               obj.type= "Biostrings")

  # Get names
  prot_names <- names(AAStringSet); prot_names
  ensembl_peptide_id <- unlist(lapply(prot_names, function(name) return(strsplit(name, split = " ")[[1]][1])))
  
  # Only remove version number if dealing with mouse.
  if (species %in% c("mouse", "human")) {
    ensembl_peptide_id <- unlist(lapply(ensembl_peptide_id, function(id) return(strsplit(id, split = "\\.")[[1]][1]))) # Remove version number
  }
  
  # Debugging
    ensembl_peptide_id <- ensembl_peptide_id[1:1000]
    ensembl_peptide_id
  ...
  seqs <- getSequence(id = ensembl_peptide_id, 
                      type="with_refseq_peptide",
                      seqType="peptide",
                      mart=ensembl) 
  grep("Variant", listFilters(ensembl))
  seqs
