...
candidate_AA_vec = c("Q", "P", "R", "A", "H")
species_vec = c("fly", "worm", "mouse")
...
filtered_out_cats <- get(term, GOCCOFFSPRING) 

# Remove any gene that has an annotation in the nucleus or one of its offspring.
...
for (species in species_vec) {
  ...
  # Is there a way to do this better so we are not downloading repeats?  
...
for (species in species_vec) {
  ...
  for (candidate_AA in candidate_AA_vec) {
    ...
    # 1.) Hard coded training
      training_set_ids <- test_id_dict[["mouse_Q"]]
    ...
    # ###########################################
    short <- c("BP", "MF", "CC")
    long <- c("biological_process", "molecular_function", "cellular_component")
    short_to_long <-  setNames(long, short)
    for (go_ont in short) {
      # go_ont <- "BP"
      print(paste0("Doing enrichment analysis for ", short_to_long[go_ont]))
      
      # Only allow proteins that have annotations in current ontology to be in background universe.
      proteins_has_ont_type <- proteins %>% filter(go_type == short_to_long[go_ont])
